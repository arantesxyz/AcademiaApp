...
export class Student extends Component {
    ...
    async _deleteStudent() {
            ...
            let response = {};
        try {
            let path = {"/student/" : String + this.state.student._id}
    ...
    async _setStateStudent() {
        ...
        let response = {};
    try {
        let path = {"/student/" : String + this.props.match.params.id}
                        ...
                        <tbody>
                             {this.state.student.length > 0 &&
                                    <tr key={this.state.student._id}>
                                        ...
                                        <td>
                                            <a href="#" title="Editar">
                                            <Link to={`/alunos/editar/${this.state.student._id}`}> 
                                                <FontAwesomeIcon icon="edit" />{" "}
                                                </Link>
                                            </a>
                                            <a href="#" title="Deletar" onClick={(e) => this.confirmDelete(window.confirm({"Deletar aluno:":String + this.state.student._id}))}>
